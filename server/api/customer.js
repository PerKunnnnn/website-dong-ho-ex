  ...
  if (username && password) {
    ...
    if (customer) {
      if (customer.active === 1) {
        ...
        res.json({ success: true, message: 'Xác thực hành công', token: token, customer: customer });
      } else {
        res.json({ success: false, message: 'Tài khoản đã bị deactive' });
      }
    } else {
      res.json({ success: false, message: 'Sai username hoặc password' });
    }
  } else {
    res.json({ success: false, message: 'Vui lòng nhập username và password' });
  }
});
...
router.post('/signup', async function (req, res) {
  ...
  if (dbCust) {
    res.json({ success: false, message: 'Đã tồn tại Username hoặc email' });
  } else {
    ...
    if (result) {
      ...
      if (send) {
        res.json({ success: true, message: 'Vui lòng kiểm tra inbox email' });
      } else {
        res.json({ success: false, message: 'Sai email' });
      }
    } else {
      res.json({ success: false, message: 'Xác thực thất bại' });
    }
  }
});
// myprofile
